...
## 추상 팩토리 패턴
- 소프트웨어 개발에서 상위 클래스에서 객체를 생성하는 공통 인터페이스를 제공하면서 하위 클래스가 생성될 객체의 타입을 변경할 수 있도록 하는 디자인 패턴
- 객체 지향 프로그래밍의 문맥에서 주로 사용
- 공통 테마를 가진 개별 팩토리의 그룹을 캡슐화하는 방법을 제공
- 구체적인 클래스를 지정하지 않으면서 관련되거나 종속적인 객체를 생성 하는 인터페이스 제공

ex) 회원 계정에서 공통을 만들고 회원, 관리자, 특수 멤버 등을 유저 타입에 따라 생성하도록 
